...
class ImageClassifier():
	def __init__(self,
		...
		self.height_reduction_ratio = height_reduction_ratio
	...
	def crop_image(self, image):
		...
		:param image:
		:param height_reduction_ratio: proportion de hauteur d'image à enlever en haut et en bas
		:return:
		"""
		height = image.height
		height_reduction = height * self.height_reduction_ratio
		image = image.crop((0, height_reduction, image.width, height - height_reduction))
		return image
	...
	def pad_and_resize(self, image):
		...
		padded = Image.new(image.mode, (new_width, new_height), (255,))
		padded.paste(image, (padding_x, padding_y))
		resized = padded.resize((self.resize[0], self.resize[1]))
		return resized
	...
	def hog_label_from_image(self, image_path, produce_labels=False, show_image=True):
		label = self.reverse_vocab.get(image_path.split("/")[-2])
		# if self.vocab[label] != "deleted":
		# 	return
		...
		padded_image = self.pad_and_resize(cropped)

		if show_image:
			Image.Image.show(padded_image)
			exit()
		features = self.extract_features(padded_image, show_features=False)
		if produce_labels:
	...
	def retrieve_hog_and_label(self, image):
		try:
			return self.hog_label_from_image(image, produce_labels=True, show_image=False)
		except OSError:
			return None
	...
	def train(self,
		...
		# lsprint(classification_report(y_pred, y_test))
	...
	def predict_line_dir(self, image):
		print("STARTING")
		features = self.hog_label_from_image(image_path=image, show_image=False)
		prediction = self.model.predict([features])
		prediction_as_label = self.vocab[prediction[0]]
		try:
			os.mkdir(f"predictions/")
		except FileExistsError:
			pass
		try:
			os.mkdir(f"predictions/{prediction_as_label}")
		except FileExistsError:
			pass
		shutil.copyfile(image, f"predictions/{prediction_as_label}/{image.split('/')[-1]}")
		return prediction_as_label
	...
	def predict_to_dir(self,
		...
		for image in images:
			...
			try:
				os.mkdir(f"test_predictions/")
			except FileExistsError:
				pass
			try:
				os.mkdir(f"test_predictions/{out_dir}")
			except FileExistsError:
				pass
			shutil.move(image, f"test_predictions/{out_dir}/{image.split('/')[-1]}")
...
if __name__ == '__main__':
	line_classifier = ImageClassifier(build_vocab=True,
									  corpus_path="data/lines/*",
									  binary_corpus_path="data/corpus_lines.data",
									  max_size=(2880,108),
										orientations=8,
										pixels_per_cell=(16, 2),
										cells_per_block=(2, 4),
									  resize=(2472, 89),
									  height_reduction_ratio=0.25,
									  model_path="models/LinesDeletionClassifierRF.joblib",
									  vocab_path="models/LinesDeletionClassifierRF.voc",
									  workers=int(sys.argv[1]))
	line_classifier.train()

	char_classifier = ImageClassifier(build_vocab=True,
									  corpus_path="data/chars/*",
									  binary_corpus_path="data/corpus_chars.data",
									  max_size=(20, 60),
									  resize=(21, 60),
										orientations=8,
										pixels_per_cell=(5, 10),
										cells_per_block=(4, 4),
									  height_reduction_ratio=0.05,
									  model_path="models/CharsDeletionClassifierRF.joblib",
									  vocab_path="models/CharsDeletionClassifierRF.voc",
									  workers=int(sys.argv[1]))
	char_classifier.train()
